...
def wait_for_working_hours(start_time, end_time):
    while True:
        ...
        if now < start:
            ...
            time_to_wait = (datetime.datetime.combine(datetime.date.today(), now) - datetime.datetime.now(tz = None)).total_seconds()
            print(f"Waiting {time_to_wait} seconds until working hours start... {now}")
            ...
            print(f"Working hours are over. Waiting {time_to_wait} seconds until tomorrow...    {now}")
            time.sleep(time_to_wait)
        else:
            # working hours are ongoing
            start_plus_6_hours = datetime.datetime.combine(datetime.date.today(), now) + datetime.timedelta(hours=6)
            if start_plus_6_hours < datetime.datetime.now():
                ...
                print(f"Working hours are ongoing, but it's too late to update leads. Waiting {time_to_wait} seconds until tomorrow...  {now}")
                time.sleep(time_to_wait)
            else:
                ...
                print(f"Working hours are ongoing. Waiting {time_to_wait} seconds before updating leads...  {now}")
                time.sleep(time_to_wait)
                # update the leads
                print(f"Updating leads...    {now}")
                # wait for another 6 hours before updating leads again
                time.sleep(6 * 60 * 60)
                print(f"Waiting for 6 hours within working hours...  {now}")

wait_for_working_hours("10:00", "19:00") #24hr clock 
